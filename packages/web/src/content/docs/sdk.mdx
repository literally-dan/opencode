...
| `session.abort({ path })`                                  | Abort a running session            | Returns `boolean`. Query `scope`: `turn` stops only the running turn, and background tasks keep running. `all` (default) also cancels background tasks                                                                   |
