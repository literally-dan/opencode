...
The default is `2`, which allows primary agents to launch subagents and lets a coordinating subagent delegate one additional level — for example the `/target` coordinator delegating to a worker and a reviewer. Set it to `1` to prevent subagents from launching subagents, or `0` to prevent all subagent launches.
