...
export const FinalCallInstruction =
  "Tools are no longer available in this Ask. Do not call any tools. Answer the question now with text only, using the context and tool results above. If something is still unknown, say what is missing."

export const ToolLimitNote =
  "[Ask stopped before an answer: tools were no longer available and the model asked for another tool. Send a follow-up to continue.]"

export const PermissionRejectedNote = "[Ask stopped: permission was rejected. Send a follow-up to continue.]"
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const askImpl = Effect.fnUntraced(function* (
      ...
      const normal = yield* MessageV2.stream(input.sessionID).pipe(
        Effect.provideService(Database.Service, database),
        Effect.map((messages) => {
          // The stream is newest-first. Only the newest assistant message can still be running, and model conversion
          // would report its unresolved tool calls as interrupted. An older one without time.completed was left by a
          // crash and stays visible.
          const newest = messages.find((message) => message.info.role === "assistant")?.info
          const running = newest?.role === "assistant" && newest.time.completed === undefined ? newest.id : undefined
          return MessageV2.filterCompacted(beforeRevert(messages, session.revert)).filter(
            (message) => message.info.id !== running,
          )
        }),
      )
      ...
      // Like the SessionPrompt max-steps step, the final call keeps the tool list and toolChoice and adds an instruction.
      // With toolChoice "none", providers can drop the tools, and Bedrock also strips tool history.
      // The final definitions have no execute handlers, so a tool call on the final call does not run.
      const definitions = Object.fromEntries(
        Object.entries(tools).map(([id, item]) => [id, { ...item, execute: undefined }]),
      )
      const answer: string[] = []

      for (let call = 0; call < MAX_PROVIDER_CALLS; call++) {
        const final = call === MAX_PROVIDER_CALLS - 1
        const enabledTools = final ? definitions : tools
        ...
        const streamed = yield* llm
          .stream({
            ...
            sessionID: input.sessionID,
            retries: 2,
            // A trailing assistant message is prefill, which newer Claude models reject.
            messages: final ? [...modelMessages, { role: "user", content: FinalCallInstruction }] : modelMessages,
          })
          .pipe(
            Stream.runCollect,
        ...
        const roundText = streamed
          .filter(LLMEvent.is.textDelta)
          .map((event) => event.text)
          .join("")
        answer.push(roundText)
        synthetic.push(round.message)
        if (final && round.calledTool && !roundText.trim())
          answer.push(answer.join("").trim() ? `\n\n${ToolLimitNote}` : ToolLimitNote)
        if (!round.calledTool || final) break
        if (stop.rejected && cfg.experimental?.continue_loop_on_deny !== true) {
          if (!answer.join("").trim()) answer.push(PermissionRejectedNote)
          break
        }
      }
    ...
    const unsubscribe = yield* events.listen((event) => {
      ...
      return Option.isSome(data) ? cancel({ sessionID: data.value.sessionID }).pipe(Effect.orDie) : Effect.void
    })
...
// Removes what SessionRevert.cleanup removes before the next normal prompt.
// Messages are newest-first, as returned by MessageV2.stream.
function beforeRevert(messages: SessionV1.WithParts[], revert: Session.Info["revert"]) {
  if (!revert) return messages
  const index = messages.findIndex((message) => message.info.id === revert.messageID)
  if (index < 0) return messages
  const older = messages.slice(index + 1)
  if (!revert.partID) return older
  const target = messages[index]
  const part = target.parts.findIndex((item) => item.id === revert.partID)
  return [{ ...target, parts: part < 0 ? target.parts : target.parts.slice(0, part) }, ...older]
}
...
function userMessage(input: {
  ...
  return {
    ...
    parts: [{ id: input.partID, messageID: input.id, sessionID: input.sessionID, type: "text", text: input.question }],
  }
}
...
function turnMessages(
  ...
  // User message stamps derive from message IDs. Row-derived IDs keep the provider prompt prefix stable across follow-ups.
  const user = userMessage({
    id: MessageID.make(`msg_${row.id}_question`),
    partID: PartID.make(`prt_${row.id}_question`),
    sessionID,
  ...
  const id = MessageID.make(`msg_${row.id}_answer`)
  return [
    user,
    {
      info: assistantInfo({
      ...
      parts: [{ id: PartID.make(`prt_${row.id}_answer`), messageID: id, sessionID, type: "text", text: row.answer }],
    },
  ]
}
              ...
              const result = yield* definition.execute(toRecord(args), {
                ...
                ask: (request) =>
                  input.permission
                    ...
                    .pipe(
                      // Like normal Session execution, a plain rejection stops after this provider turn.
                      // Rejection with feedback stays a tool error that the model can act on.
                      Effect.tapErrorTag("PermissionRejectedError", () =>
                        Effect.sync(() => {
                          input.stop.rejected = true
                        }),
                      ),
                      Effect.orDie,
                    ),
