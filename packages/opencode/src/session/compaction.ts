  ...
  return (stored: SessionV1.WithParts) => {
    ...
    return message.parts
      .flatMap((part) => {
        ...
        if (compacted) return [call, compacted]
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const processCompactionUnlocked = Effect.fn("SessionCompaction.processUnlocked")(function* (input: ProcessInput) {
      ...
      const tailBudget =
        (cfg.compaction?.tail_turns ?? DEFAULT_TAIL_TURNS) <= 0
          ? 0
          : Math.min(preserveRecentBudget({ cfg, model: contextModel }), contextAvailable)
      // The stored summary shares the next request with fixed overhead and the
      // preserved tail. Models follow length targets loosely, so ask for half of
      // the remaining room at about two estimated tokens per word.
      const summaryWords =
        contextBudget.status === "available"
          ? Math.min(MAX_SUMMARY_WORDS, Math.max(MIN_SUMMARY_WORDS, Math.floor((contextAvailable - tailBudget) / 4)))
          : MAX_SUMMARY_WORDS
      ...
      const nextPrompt =
        ...
        [
          ...
          `Keep the whole summary under about ${summaryWords} words.`,
          ...compacting.context,
        ]
          .filter(Boolean)
          .join("\n\n")
      ...
      return yield* Effect.gen(function* () {
        ...
        if (result === "continue") {
          ...
          const projected = structuredClone(
            all.filter((message) => message.info.id === processor.message.id || snapshot.has(message.info.id)),
          )
