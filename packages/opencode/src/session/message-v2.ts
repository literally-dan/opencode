...
// surfaces. Header and chunk timeouts wait for the provider timeout on every
// attempt, so SessionRetry gives them a lower retry limit.
//
      ...
      return new APIError(
        {
          ...
          metadata: {
            code: e.name,
            ...(e.timeout ? { timeout: "true" } : {}),
          },
        },
        { cause: e },
      ).toObject()
    // provider-utils >= 4.0.50 wraps a failed mid-stream read in a 200
    // APICallError marked not retryable, and the AI Gateway wraps fetch failures
    // in its own error. Classify the timeout itself so the wrapper cannot hide it.
    // A permanent transport code in the chain still wins.
    case e instanceof Error &&
      permanent === undefined &&
      (e.cause instanceof ProviderError.ResponseStreamError || e.cause instanceof ProviderError.HeaderTimeoutError):
      return classify(e.cause, ctx)
