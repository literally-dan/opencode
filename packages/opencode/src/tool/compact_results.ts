        ...
        Effect.gen(function* () {
          ...
          return yield* withCompactionLock(
            ...
            Effect.gen(function* () {
              ...
              const durable = indexParts(messages, ctx.messageID)
              // Durable history also holds parts hidden by native compaction.
              // Select only parts the model can see; split-group checks still
              // use every durable part.
              const visible = new Set<string>(ctx.messages.flatMap((message) => message.parts.map((part) => part.id)))
                ...
                if (!found) {
                  outcomes.push({
                    ...
                    reason: ambiguous.has(partID)
                      ...
                      : durable.located.has(partID)
                        ? "not visible in current context"
