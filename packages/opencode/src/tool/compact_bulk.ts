        ...
        Effect.gen(function* () {
          ...
          return yield* withCompactionLock(
            ...
            Effect.gen(function* () {
              ...
              const visible = new Set(ctx.messages.flatMap((message) => message.parts.map((part) => part.id)))
              ...
              for (const [messageIndex, msg] of messages.entries()) {
                for (const part of msg.parts) {
                  ...
                  if (!visible.has(part.id)) {
                    skipped.push({ partID: part.id, status: "skipped", reason: "not visible in current context" })
                    continue
                  }
