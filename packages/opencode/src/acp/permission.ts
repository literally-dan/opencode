...
export class Handler {
  ...
  private async route(event: PermissionEvent, signal: AbortSignal, directory?: string) {
    ...
    // The request is pending in the Instance that raised it. A nested leaf can live
    // in another location than the managed session, for example after the managed session moved.
    const target = directory ?? session.cwd
    ...
    const next = previous
      .then(() => (signal.aborted ? undefined : this.process(event, session, target, signal)))
      .catch((error) => (signal.aborted ? undefined : this.rejectOwnedPermission(event, error, target, signal)))
      .finally(() => {
  ...
  private async process(event: PermissionEvent, session: ACPSession.Info, directory: string, signal: AbortSignal) {
    const permission = event.properties
    ...
    if (!this.input.connection.requestPermission) {
      await this.reply(permission.id, "reject", signal, directory)
      return
    }
    ...
    await this.reply(permission.id, reply, signal, directory)
  }
