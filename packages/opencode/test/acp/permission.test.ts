...
describe("acp permissions", () => {
  ...
  it("rejects when requestPermission fails", async () => {
    ...
    harness.subscription.handle(permissionAsked("ses_a", "perm_failed"), "/instance")

    await pollUntil(() => harness.replies.length === 1, "failed permission was never rejected")

    expect(harness.replies[0]).toMatchObject({ requestID: "perm_failed", reply: "reject", directory: "/instance" })
  })
  ...
  it("routes a nested permission through a managed ancestor that moved to another location", async () => {
    const harness = createHarness({
      sessionGet: async (sessionID) => {
        if (sessionID === "ses_leaf") return { parentID: "ses_root", taskParentID: "ses_root" }
        if (sessionID === "ses_root") return { workspaceID: "moved", directory: "/moved", path: "moved" }
        throw new Error(`unexpected session lookup: ${sessionID}`)
      },
    })
    await createSession(harness.session, "ses_root", "/moved")

    harness.subscription.handle(permissionAsked("ses_leaf", "perm_moved"), "/workspace")

    await pollUntil(() => harness.replies.length === 1, "permission from a moved ancestor was never replied")
    expect(harness.requests[0]?.sessionId).toBe("ses_root")
    // The request is pending in the leaf's Instance, not in the managed session's new location.
    expect(harness.replies[0]).toMatchObject({ requestID: "perm_moved", reply: "once", directory: "/workspace" })
  })
