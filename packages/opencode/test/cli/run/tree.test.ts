...
describe("run session tree", () => {
  ...
  test("excludes descendants whose Task owner differs from their parent", async () => {
    const sdk = client({
      children: async (sessionID) =>
        sessionID === "root" ? [taskInfo("claimed", "root", { taskParentID: "other" })] : [],
    })

    expect(await fetchDescendants(sdk, "root")).toEqual([])
  })

  test("excludes Task descendants from another project", async () => {
    const sdk = client({
      children: async (sessionID) =>
        sessionID === "root" ? [taskInfo("foreign", "root", { projectID: "foreign" })] : [],
    })

    expect(await fetchDescendants(sdk, "root")).toEqual([])
  })

  test("keeps Task descendants after the root moves to another location", async () => {
    const sdk = client({
      get: async (sessionID) =>
        sessionID === "root" ? { workspaceID: "moved", directory: "/moved", path: "moved" } : {},
      children: async (sessionID) => {
        if (sessionID === "root") return [taskInfo("child", "root")]
        if (sessionID === "child") return [taskInfo("grandchild", "child")]
        return []
    ...
    expect(await fetchDescendants(sdk, "root")).toEqual([{ id: "child" }, { id: "grandchild" }])
  })
  ...
  test("rejects a parent that differs from the Task owner", async () => {
    const sdk = client({
      get: async (sessionID) =>
        sessionID === "leaf" ? { parentID: "root", taskParentID: "other" } : { parentID: undefined },
    })

    expect(await lookupSessionTreeOwnership(sdk, new Set(["root"]), "leaf")).toEqual({ type: "foreign" })
  })

  test("rejects Task ancestry across a project boundary", async () => {
    const sdk = client({
      get: async (sessionID) => ({
        parentID: sessionID === "leaf" ? "root" : undefined,
        taskParentID: sessionID === "leaf" ? "root" : undefined,
        projectID: sessionID === "leaf" ? "foreign" : "project",
      }),
  ...
  test("keeps a descendant owned after its root moves to another location", async () => {
    let moved = false
    const sdk = client({
      get: async (sessionID) => ({
        parentID: sessionID === "leaf" ? "root" : undefined,
        taskParentID: sessionID === "leaf" ? "root" : undefined,
        ...(sessionID === "root" && moved ? { workspaceID: "moved", directory: "/moved", path: "moved" } : {}),
      }),
    })
    const tree = new Set(["root"])
