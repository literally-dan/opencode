...
it.instance(
  "reply - normal reject cancels normal permissions and leaves pending Ask permissions",
  () =>
    Effect.gen(function* () {
      const sessionID = SessionID.make("session_normal_reject")
      const normalID = PermissionV1.ID.make("per_normal_reject")
      const otherNormalID = PermissionV1.ID.make("per_normal_reject_other")
      ...
      expect(yield* fail(Fiber.join(otherNormal))).toBeInstanceOf(PermissionV1.RejectedError)
      expect((yield* list()).map((item) => item.id)).toEqual([askID])

      yield* reply({ requestID: askID, reply: "once" })
      yield* Fiber.join(askPending)
    }),
  { git: true },
)
