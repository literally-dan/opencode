...
// Hides the seeded turn behind a completed native compaction, then adds one
// visible tool result and a live request. Returns the model-visible messages.
const hideSeed = Effect.fn("HistoryTest.hideSeed")(function* (sessionID: SessionID) {
  const session = yield* Session.Service
  ...
  const assistant = (parentID: MessageID, summary: boolean) =>
  ...
  const summary = yield* assistant(boundary.id, true)
  yield* session.updatePart({
    id: PartID.ascending(),
    messageID: summary.id,
    sessionID,
    type: "text",
    text: "The seeded turn ran bash and read notes.",
  } satisfies SessionV1.TextPart)
  const followup = yield* assistant(boundary.id, false)
  const visibleTool = yield* session.updatePart({
    id: PartID.ascending(),
    ...
    state: {
      status: "completed",
      input: { pattern: "TODO" },
      output: `src/index.ts: TODO ${"visible grep detail ".repeat(20)}`,
      title: "grep",
      metadata: {},
      time: { start: 7, end: 8 },
    },
  ...
  return {
    visible: MessageV2.filterCompacted((yield* session.messages({ sessionID })).toReversed()),
    visibleTool,
  }
})
...
describe("tool.compact_results", () => {
  ...
  it.instance("selects only parts visible after an earlier native compaction", () =>
    Effect.gen(function* () {
      const { chat, bashPart, readPart } = yield* seed()
      const session = yield* Session.Service
      ...
      const explicit = yield* def.execute(
        { summary: "hidden bash output", part_ids: [bashPart.id] },
        withMessages(chat.id, hidden.visible),
      )
      const selected = yield* def.execute(
        { summary: "tool results", select: { types: ["tool"] } },
        withMessages(chat.id, hidden.visible),
      )

      expect(explicit.metadata).toMatchObject({ compacted: 0, skipped: 1 })
      expect(explicit.output).toContain(`${bashPart.id} → skipped: not visible in current context`)
      expect(selected.metadata).toMatchObject({ compacted: 1, skipped: 0 })
      expect(selected.output).toContain(hidden.visibleTool.id)
      const stored = yield* Effect.forEach([bashPart.id, readPart.id, hidden.visibleTool.id], (partID) =>
        session.findPart({ sessionID: chat.id, partID }),
      )
      expect(stored.map((part) => part !== undefined && isCompacted(part))).toEqual([false, false, true])
    }),
  )
})
