...
describe("session.compaction.process", () => {
  ...
  for (const item of [
    // 28k usable - 16k overhead - 7k preserved tail leaves 5k tokens: half of
    // that at two tokens per word.
    { name: "small", model: createModel({ context: 32_000, output: 4_000 }), words: 1250 },
    { name: "large", model: createModel({ context: 1_000_000, output: 32_000 }), words: 2000 },
  ]) {
    itCompaction.instance(
      `asks for a summary that fits the ${item.name} model's context budget`,
      () => {
        const stub = llm()
        let captured = ""
        stub.push(reply("summary", (input) => (captured = JSON.stringify(input.messages))))
        return Effect.gen(function* () {
          const ssn = yield* SessionNs.Service
          const session = yield* ssn.create({})
          yield* createUserMessage(session.id, "history ".repeat(1_000))
          yield* createCompactionMarker(session.id)
          const msgs = yield* ssn.messages({ sessionID: session.id })
          ...
          yield* SessionCompaction.use.process({
            ...
            sessionID: session.id,
            auto: true,
            requestOverhead: 16_000,
          })

          expect(captured).toContain(`Keep the whole summary under about ${item.words} words.`)
        }).pipe(
          withCompaction({ llm: stub.llmLayer, provider: ProviderTest.fake({ model: item.model }), config: cfg() }),
        )
      },
      { git: true },
    )
  ...
  itCompaction.instance(
    "stores an oversized native summary intact for the next compaction",
    () => {
      const stub = llm()
      ...
      stub.push(
        Stream.make(
          LLMEvent.textStart({ id: "txt-0" }),
          LLMEvent.textDelta({ id: "txt-0", text: objective }),
          LLMEvent.textEnd({ id: "txt-0" }),
          LLMEvent.textStart({ id: "txt-1" }),
          LLMEvent.textDelta({ id: "txt-1", text: nextMove }),
          LLMEvent.textEnd({ id: "txt-1" }),
          LLMEvent.stepFinish({ index: 0, reason: "stop", usage: basicUsage() }),
          LLMEvent.finish({ reason: "stop", usage: basicUsage() }),
        ),
      )
      let captured = ""
      stub.push(reply("second summary", (input) => (captured = JSON.stringify(input.messages))))
      return Effect.gen(function* () {
        const ssn = yield* SessionNs.Service
        const session = yield* ssn.create({})
        ...
        const firstResult = yield* SessionCompaction.use.process({
          parentID: first.at(-1)!.info.id,
          messages: first,
          sessionID: session.id,
          auto: false,
        })
        ...
        yield* createUserMessage(session.id, "later work ".repeat(2_000))
        yield* createCompactionMarker(session.id)
        const second = MessageV2.filterCompacted((yield* ssn.messages({ sessionID: session.id })).toReversed())
        const secondResult = yield* SessionCompaction.use.process({
          parentID: second.at(-1)!.info.id,
          messages: second,
          sessionID: session.id,
          auto: false,
        })
        ...
        expect(generated?.parts.flatMap((part) => (part.type === "text" ? [part.text] : []))).toEqual([
          objective,
          nextMove,
        ])
        expect(secondResult).toBe("continue")
        expect(captured).toContain("NEXT_MOVE_MARKER")
      }).pipe(withCompaction({ llm: stub.llmLayer, config: cfg({ tail_turns: 0 }) }))
    },
    { git: true },
  )
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        const test = yield* TestInstance
        const session = yield* ssn.create({})
        const history = yield* createUserMessage(session.id, "h".repeat(100_000))
        ...
        const text = generated?.parts
          ...
          .map((part) => part.text)
          .join("")
        // The stored summary is what the next request replays, so an oversized
        // one fails the size checks instead of being cut down to pass them.
        expect(result).toBe("stop")
        expect(generated?.info.role === "assistant" ? JSON.stringify(generated.info.error) : "").toContain(
          "still leaves about",
        )
        expect(text).toBe("s".repeat(100_000))
        expect(MessageV2.filterCompacted(all.toReversed()).map((message) => message.info.id)).toContain(history.id)
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        // The call signature stays, like it does in the live projection, so the
        // summary still records which tools ran.
        expect(captured).toContain(
          '[Assistant tool call]: read({\\"secret\\":\\"ORIGINAL_INPUT_SECRET\\"})\\n[Tool]: [Compacted',
        )
        expect(captured).toContain(
          '[Assistant tool call]: read({\\"secret\\":\\"UNSUMMARIZED_INPUT_SECRET\\"})\\n[Tool]: [Compacted',
        )
