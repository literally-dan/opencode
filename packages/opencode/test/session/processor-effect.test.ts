...
const providerToolRetryLLM = Layer.effect(
  LLM.Service,
  Effect.gen(function* () {
    const state = yield* RetryAccountingState
    return LLM.Service.of({
      stream: () => {
        ...
        if (state.calls === 1)
          return Stream.make(
            LLMEvent.stepStart({ index: 0 }),
            LLMEvent.toolInputStart({ id: "search-1", name: "web_search" }),
            LLMEvent.toolCall({ id: "search-1", name: "web_search", input: { query: "news" }, providerExecuted: true }),
            LLMEvent.providerError({ message: "terminated" }),
          )
        return Stream.make(
          LLMEvent.stepStart({ index: 0 }),
          LLMEvent.textStart({ id: "text-1" }),
          ...
          LLMEvent.stepFinish({ index: 0, reason: "stop", usage: successfulUsage }),
          LLMEvent.finish({ reason: "stop", usage: successfulUsage }),
        )
      },
    })
  }),
)
const providerToolRetryLLMNode = LayerNode.make({
  service: LLM.Service,
  layer: providerToolRetryLLM,
  deps: [retryAccountingStateNode],
})
const providerToolRetryEnv = LayerNode.compile(LayerNode.group([root, retryAccountingStateNode]), [
  ...replacements,
  [LLM.node, providerToolRetryLLMNode],
])
const itProviderToolRetry = testEffect(providerToolRetryEnv)
    ...
    return LLM.Service.of({
      stream: (input) => {
        ...
        return events.pipe(
          ...
          // The AI SDK reports a rejected duplicate call, or an execution that
          // interrupted itself, as a tool-error for the admitted call ID.
          Stream.concat(
            state.mode === "duplicate" || state.mode === "interrupt"
              ? Stream.make(LLMEvent.toolError({ id: "call-1", name: "lookup", message: "tool execution rejected" }))
              : Stream.empty,
          ),
          Stream.concat(
...
// The provider reports an overflowing step while the local lookup is still
// running, so the processor requests compaction at step-finish.
const overflowUsage = new Usage({ inputTokens: 95_000, outputTokens: 0, totalTokens: 95_000 })
const nativeLLM = Layer.succeed(
  LLM.Service,
  LLM.Service.of({
    stream: (input) => {
      const native = LLMNativeRuntime.stream({
        model: {
          ...
          api: { ...input.model.api, npm: "@ai-sdk/openai" },
        },
        provider: {
        ...
        llmClient: {
          ...
          stream: () =>
            Stream.make(
              LLMEvent.stepStart({ index: 0 }),
              LLMEvent.toolCall({ id: "call-1", name: "lookup", input: {} }),
              LLMEvent.stepFinish({ index: 0, reason: "tool-calls", usage: overflowUsage }),
              LLMEvent.finish({ reason: "tool-calls", usage: overflowUsage }),
            ),
        } as LLMClientShape,
        messages: [],
        tools: input.tools,
      ...
      return native.type === "supported" ? native.stream : Stream.fail(new Error(native.reason))
    },
  }),
)
const nativeEnv = LayerNode.compile(
  LayerNode.group([root, recoveryStateNode, Plugin.node, Permission.node, RuntimeFlags.node]),
  [...replacements, [LLM.node, nativeLLM], [Image.node, recoveryImageNode]],
)
  ...
  const held: Tool.Def<typeof emptyParameters> = {
    ...
    execute: (_args, ctx) =>
      Effect.gen(function* () {
        ...
        if (state.mode === "interrupt" || state.mode === "interrupt-drop") return yield* Effect.interrupt
        if (state.mode === "abort-interrupt") {
          yield* Deferred.await(state.cancelled)
          return yield* Effect.interrupt
        }
        yield* Deferred.await(state.release)
        ctx.abort.removeEventListener("abort", onAbort)
        yield* Deferred.succeed(state.finished, undefined)
        // Permission prompts die with the rejection, like ctx.ask does.
        if (state.mode === "rejected") return yield* Effect.die(new PermissionV1.RejectedError())
        return {
    ...
    (dir) =>
      Effect.gen(function* () {
        ...
        const off = yield* events.listen((event) => {
          if (event.type !== SessionStatus.Event.Status.type) return Effect.void
          const data = event.data as typeof SessionStatus.Event.Status.data.Type
          if (data.sessionID === setup.input.sessionID && data.status.type === "retry") retries.push(data.status.attempt)
          return Effect.void
        })
        const run = yield* setup.handle.process(setup.input).pipe(Effect.forkChild)
        ...
        expect(value).toBe("continue")
        // The stream failure is retryable, but an executed tool cannot be
        // replayed, so no retry countdown may be shown while it settles.
  ...
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        ...
        // A direct settlement callback is the tool wrapper's report, applied once
        // the call registers. It settled the call, so recovery has nothing left
        // to wait for.
        expect(value).toBe("continue")
        expect(setup.handle.message.error).toBeUndefined()
        expect(setup.state.providerCalls).toBe(1)
...
itRecovery.live("session.processor effect tests wait for an admitted tool after a duplicate ID error", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const setup = yield* recoverySetup(path.resolve(dir))
        setup.state.mode = "duplicate"
        const run = yield* setup.handle.process(setup.input).pipe(Effect.forkChild)

        yield* Deferred.await(setup.state.started)
        // The stream's tool-error belongs to the rejected duplicate. The admitted
        // execution still runs, so recovery must keep waiting for its own result.
        const early = yield* Fiber.join(run).pipe(Effect.timeoutOption("1 second"))
...
itRecovery.live("session.processor effect tests settle a self-interrupted admitted tool as failed", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const setup = yield* recoverySetup(path.resolve(dir))
        setup.state.mode = "interrupt"
        ...
        // The stream's tool-error is ignored for admitted calls, so the wrapper
        // reports the interruption itself.
        expect(value).toBe("continue")
        expect(call?.state.status).toBe("error")
        if (call?.state.status === "error") expect(call.state.error).toBe("Tool execution interrupted")
      }),
    { config: cfg },
  ),
)

itRecovery.live("session.processor effect tests recover after a self-interrupted tool and stream failure", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const setup = yield* recoverySetup(path.resolve(dir))
        setup.state.mode = "interrupt-drop"

        // Recovery waits for every admitted call to settle. Without a settlement
        // for the interruption it would wait until the user interrupts.
        const value = yield* setup.handle.process(setup.input).pipe(Effect.timeoutOption("10 seconds"))
        const call = (yield* MessageV2.parts(setup.msg.id)).find(
          (part): part is SessionV1.ToolPart => part.type === "tool",
        )

        expect(Option.getOrUndefined(value)).toBe("continue")
        expect(setup.handle.message.error).toBeUndefined()
        expect(setup.state.providerCalls).toBe(1)
        expect(setup.state.cancellations).toBe(0)
        expect(call?.state.status).toBe("error")
...
itRecovery.live("session.processor effect tests mark a tool interrupted by processor abort as aborted", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const setup = yield* recoverySetup(path.resolve(dir))
        setup.state.mode = "abort-interrupt"
        const run = yield* setup.handle.process(setup.input).pipe(Effect.forkChild)
...
itRecovery.live("session.processor effect tests continue after sibling tool failure and stream failure", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const setup = yield* recoverySetup(path.resolve(dir))
        ...
        yield* Deferred.await(setup.state.started)
        // A failed sibling is an ordinary tool result. Recovery keeps waiting for
        // the running tool instead of halting with the transport error.
        ...
        const running = calls.find((part) => part.callID === "call-1")
        ...
        expect(running?.state.status).toBe("completed")
        if (running?.state.status === "completed") expect(running.state.output).toBe("held result")
      }),
    { config: cfg },
  ),
)

itRecovery.live("session.processor effect tests stop after a permission rejection and stream failure", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const setup = yield* recoverySetup(path.resolve(dir))
        setup.state.mode = "rejected"
        const run = yield* setup.handle.process(setup.input).pipe(Effect.forkChild)
...
itNative.live("session.processor effect tests record a slow native tool before compacting", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const setup = yield* recoverySetup(path.resolve(dir))
        ...
        yield* Deferred.await(setup.state.started)
        // The overflowing step-finish must wait for the running tool. Handled
        // first, it ends the stream and cleanup marks the tool aborted.
        const early = yield* Fiber.join(run).pipe(Effect.timeoutOption("2 seconds"))
        expect(Option.isNone(early)).toBe(true)
        expect(setup.handle.message.finish).toBe("end_turn")
        yield* Deferred.succeed(setup.state.release, undefined)
        ...
        expect(value).toBe("compact")
        expect(setup.handle.message.finish).toBe("tool-calls")
        expect(setup.state.cancellations).toBe(0)
        expect(call?.state.status).toBe("completed")
        if (call?.state.status === "completed") expect(call.state.output).toBe("held result")
      }),
...
itNative.live("session.processor effect tests interrupt a native step held for a running tool", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const setup = yield* recoverySetup(path.resolve(dir))
        ...
        yield* Deferred.await(setup.state.started)
        yield* Fiber.interrupt(run).pipe(Effect.timeout("2 seconds"))
        ...
        expect(setup.state.cancellations).toBe(1)
        expect(call?.state.status).toBe("error")
        if (call?.state.status === "error") expect(call.state.error).toBe("Tool execution aborted")
...
it.live("session.processor effect tests record aborted errors without publishing idle", () =>
  provideTmpdirServer(
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        // halt leaves idle to the Session runner, which first checks background Task leases.
        // This test runs the processor without a runner, so nothing publishes idle.
        expect(state.type).not.toBe("idle")
...
itProviderToolRetry.live("session.processor effect tests retry after a provider-executed tool call", () =>
  provideTmpdirInstance(
    (dir) =>
      Effect.gen(function* () {
        const { processors, session, provider } = yield* boot()
        const state = yield* RetryAccountingState
        const chat = yield* session.create({})
        const parent = yield* user(chat.id, "search the web")
        const msg = yield* assistant(chat.id, parent.id, path.resolve(dir))
        const mdl = yield* provider.getModel(ref.providerID, ref.modelID)
        const handle = yield* processors.create({ assistantMessage: msg, sessionID: chat.id, model: mdl })
