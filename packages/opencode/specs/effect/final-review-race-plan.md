...
- Trust a Task ancestry edge only when `taskParentID === parentID` and both
  sessions have the same project. Do not compare location: moving a Session
  does not move its Task children. Task notification targets and `task_id`
  resume still require the same location because they run in the caller's
  Instance.
...
- Update app permission auto-response and request trees, TUI request trees, and
  mini-run descendant discovery to accept only authenticated Task edges in the
  same project.
...
- Add parent-only, valid Task, cross-project, moved-root, and imported-owner regressions.
...
- Parent-only and cross-project Sessions receive no Task permission trust.
