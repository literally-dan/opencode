...
export function DialogAsk(props: DialogAskProps) {
  ...
  const sessionLocation = createMemo(() => {
    const session = sync.session.get(props.sessionID)
    return session ? { directory: session.directory, workspaceID: session.workspaceID } : undefined
  })
  ...
  // Unlike the other Ask keys, cancel stays active while a permission prompt is shown.
  useBindings(() => ({
    mode: "modal",
    enabled: permission() !== undefined || store.pane !== "composer",
    bindings: [{ key: "c", desc: "Cancel active Ask", group: "Ask", cmd: cancel }],
  }))
          ...
          >
            {(request) => (
              <box flexShrink={0} minHeight={9} maxHeight={15}>
                {/* DialogProvider renders above the app LocationProvider, so the prompt needs its own. */}
                <LocationProvider location={sessionLocation()}>
                  <PermissionPrompt request={request} directory={sessionLocation()?.directory} modal />
                </LocationProvider>
              </box>
            )}
          </Show>
