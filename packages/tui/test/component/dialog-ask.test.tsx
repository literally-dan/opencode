...
import { InputRenderable, ScrollBoxRenderable, TextareaRenderable, type Renderable } from "@opentui/core"
...
describe("DialogAsk", () => {
  ...
  test("cancels an Ask while a permission prompt is shown", async () => {
    await using tmp = await tmpdir()
    ...
    try {
      ...
      await waitForText(mounted.app, "Permission required")
      mounted.app.mockInput.pressKey("c")
      expect(await waitForText(mounted.app, "Ask cancelled")).not.toContain("Permission required")
      expect(aborts).toBe(1)
  ...
  test("mounts a fresh permission prompt when the first Ask permission changes", async () => {
    await using tmp = await tmpdir()
    const replies: Record<string, unknown>[] = []
    let activeRequestID = ""
    const mounted = await mountAsk(tmp.path, async (request) => {
    ...
    const permission = (id: string, file: string) =>
      emit(mounted.events, {
        id: `evt_${id}`,
        type: "permission.asked",
        properties: {
    ...
    try {
      ...
      mounted.app.mockInput.pressArrow("right")
      mounted.app.mockInput.pressEnter()
      expect(await waitForText(mounted.app, "Always allow")).toContain("src/a.ts")

      emit(mounted.events, {
        id: "evt_per_a_replied",
        type: "permission.replied",
        properties: { sessionID: "ses_ask", requestID: "per_a", reply: "once" },
      })
      expect(await waitForText(mounted.app, "Read src/b.ts")).not.toContain("Always allow")
      mounted.app.mockInput.pressEnter()
      await wait(() => replies.length === 1)
...
async function mountPromptModel(root: string, commands: Record<string, unknown>[] = []) {
  ...
  const mounted = await mountPromptAsk(root, async (request) => {
    const url = new URL(request.url)
    ...
    if (url.pathname === "/command") return json(commands)
...
test("Prompt routes /ask directly to the Ask endpoint", async () => {
  await using tmp = await tmpdir()
  const mounted = await mountPromptModel(tmp.path)

  try {
    mounted.prompt.set({ input: "/ask Explain routing", parts: [] })
    mounted.prompt.submit()
    await wait(() => mounted.requests.some((request) => request.method === "POST" && request.path.endsWith("/ask")))
    const posts = mounted.requests.filter((request) => request.method === "POST")
...
test("Prompt sends /ask to a user command named ask", async () => {
  await using tmp = await tmpdir()
  const mounted = await mountPromptModel(tmp.path, [
    { name: "ask", source: "command", template: "$ARGUMENTS", hints: [] },
  ])

  try {
    mounted.prompt.set({ input: "/ask Explain routing", parts: [] })
    mounted.prompt.submit()
    await wait(() => mounted.requests.some((request) => request.method === "POST"))
    const posts = mounted.requests.filter((request) => request.method === "POST")
    expect(posts.map((request) => request.path)).toEqual(["/session/ses_prompt/command"])
    expect(posts[0]?.body).toMatchObject({ command: "ask", arguments: "Explain routing" })
  } finally {
    mounted.app.renderer.destroy()
  }
})

test("Prompt hides the built-in Ask palette command when a command named ask exists", async () => {
  const search = async (commands: Record<string, unknown>[], text: string) => {
    await using tmp = await tmpdir()
    const mounted = await mountPromptModel(tmp.path, commands)
    try {
      mounted.dialog.replace(() => <CommandPaletteDialog />)
      await wait(() => mounted.app.renderer.currentFocusedEditor instanceof InputRenderable)
      await mounted.app.mockInput.typeText("Ask current")
      return await waitForText(mounted.app, text)
    } finally {
      mounted.app.renderer.destroy()
    }
  }

  expect(await search([], "Ask current context")).not.toContain("No results found")
  const skill = { name: "ask", source: "skill", template: "$ARGUMENTS", hints: [] }
  expect(await search([skill], "No results found")).not.toContain("Ask current context")
})

test("Prompt runs /ask as a shell command in shell mode", async () => {
  await using tmp = await tmpdir()
  const mounted = await mountPromptModel(tmp.path)

  try {
    await wait(() => mounted.prompt.focused)
    mounted.app.mockInput.pressKey("!")
    await waitForText(mounted.app, "exit shell mode")
    mounted.prompt.set({ input: "/ask Explain routing", parts: [] })
    mounted.prompt.submit()
    await wait(() => mounted.requests.some((request) => request.method === "POST"))
    const posts = mounted.requests.filter((request) => request.method === "POST")
    expect(posts.map((request) => request.path)).toEqual(["/session/ses_prompt/shell"])
    expect(posts[0]?.body?.command).toBe("/ask Explain routing")
  } finally {
    mounted.app.renderer.destroy()
  }
})
